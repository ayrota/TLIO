...
class MemMappedSequencesDataset(Dataset, SequencesDataset):
    ...
    def __init__(
        ...
        self.use_index_map = (
            use_index_map  # If false, calculate indices on the fly to save mem
        )
        self.keep_all_memmap_open = (
            store_in_ram or keep_all_memmap_open
        )  # If true, keep all memmap files open the whole time
    ...
    def build_index_map(self):
        ...
        self.index_map = np.empty(
            (
                sum(
                    [
                        d[base_sensor_name]["num_rows"] - self.genparams.window_size + 1
                        for d in self.data_descriptions
                    ]
                ),
                2,
            ),
        ...
        for i, desc in enumerate(self.data_descriptions):
            ...
            self.index_map[curr_idx : curr_idx + idx_rows, 1] = np.arange(
                idx_rows, dtype=np.int32
            )
            curr_idx += idx_rows
    ...
    def setup_on_the_fly_indexing(self):
        ...
        self.list_cumsum = np.cumsum(
            (data_dim - self.genparams.window_size) // self.genparams.decimator,  # + 1
            dtype=np.int64,
        )
        # self.length = (
        ...
        for i, seq_id in enumerate(self.data_list):
            ...
            for j, sensor_basename in enumerate(self.sensor_file_basenames):
                filename = os.path.join(
                    self.data_path, seq_id, sensor_basename + ".npy"
                )
                ...
                num_cols = sum(
                    [
                        ...
                        for c in sensor_desc["columns_name(width)"]
                    ]
                )
                cumulated_duration_hrs += (
                    1e-6
                    * (sensor_desc["t_end_us"] - sensor_desc["t_start_us"])
                    / 60
                    / 60
                )
                self.max_num_rows = (
                    sensor_desc["num_rows"]
                    if self.max_num_rows is None
                    else max(sensor_desc["num_rows"], self.max_num_rows)
                )
                self.min_num_rows = (
                    sensor_desc["num_rows"]
                    if self.min_num_rows is None
    ...
    def idx2tuple(self, idx):
        ...
        if not random_idx:
            ...
            max_rows = self.data_descriptions[seq_idx][self.get_base_sensor_name()][
                "num_rows"
            ]
            ...
            row = np.random.randint(
                self.list_cumsum[seq_idx]
                - (self.list_cumsum[seq_idx - 1] if seq_idx != 0 else 0)
            )
    ...
    def __getitem__(self, idx):
        ...
        ret = self.load_and_preprocess_data_chunk(
            seq_idx,
            row_in_seq,
            self.data_descriptions[seq_idx][self.get_base_sensor_name()]["num_rows"]
            - self.genparams.window_size,
        )
        return ret
    ...
    def load_data_chunk(self, seq_idx, row):
        ...
        if self.keep_all_memmap_open:
            ...
            for sensor_name, sensor_desc in seq_desc.items():
                ...
                seq_data[sensor_name] = np.load(
                    filename, mmap_mode="c"
                )  # np.memmap(filename, dtype=np.float64, mode='c',
    ...
    def get_ts_last_imu_us(self, seq_idx=0):
        ...
        if self.keep_all_memmap_open:
            ...
            num_cols = sum(
                [
                    int(c.split("(")[1].split(")")[0])
                    for c in desc["columns_name(width)"]
                ]
            )
        ...
        if not self.use_index_map:  # off-by-one sometimes between these two options
            ...
            new_len = self.list_cumsum[seq_idx] - (
                self.list_cumsum[seq_idx - 1] if seq_idx != 0 else 0
            )
            assert (
                abs(new_len - len(ts)) < 2
    ...
    def get_gt_traj_center_window_times(self, seq_idx=0):
        ...
        traj = fp[
            start_idx : end : self.genparams.decimator, -10:-3
        ]  # [start:stop:step]
        if not self.use_index_map:  # off-by-one sometimes between these two options
            # new_len = (len(fp) - self.genparams.window_size + 1) // self.genparams.decimator
            new_len = self.list_cumsum[seq_idx] - (
