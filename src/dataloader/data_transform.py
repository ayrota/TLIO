...
class TransformAddNoiseBias:
    def __init__(
        ...
        gyro_bias_range,
        mag_bias_range,
        barom_press_bias_range,
        barom_temp_bias_range,
    ):
    ...
    def __call__(self, sample):
        ...
        feats_new = {
            k: v for k, v in sample["feats"].items()
        }  # Dict of sensor_name: [ts_normalized, sensor_data]
        ...
        for sensor, feat in feats_new.items():
            ...
            if "imu" in sensor:
                ...
                # add gaussian noise
                feat_aug[:, :3, :] += (
                    torch.randn(N, 3, T, device=feat.device, dtype=feat.dtype)
                    * self.gyro_noise_std
                )
                feat_aug[:, 3:6, :] += (
                    torch.randn(N, 3, T, device=feat.device, dtype=feat.dtype)
                ...
                feat_aug[:, 0:1, :] += (
                    ...
                    * self.barom_press_bias_range
                    / 0.5
                )
                feat_aug[:, 1:2, :] += (
                    (torch.rand(N, 1, 1, device=feat.device, dtype=feat.dtype) - 0.5)
                    * self.barom_temp_bias_range
                    / 0.5
                )
...
class TransformInYawPlane:
    ...
    def __call__(self, sample):
        ...
        rand_unif = (
            2 * torch.rand((N), device=sample["ts_us"].device, dtype=torch.float32) - 1
        )  # in [-1,1]
        ...
        sample_new = {
            ...
            if k.split("second_")[-1].split("_same")[0]
            not in ["feats", "targ_dt_World", "vel_World"]
        }
        sample_new["feats"] = feats_new
        ...
        for k in "targ_dt_World", "vel_World":
            sample_new[k] = torch.einsum(
                "nik,ntk->nti", R_newWorld_from_oldWorld, sample[k]
            )
